...
void main(void)
{
    ...
    Select_ADC_Channel(0);          // Select input channel RA0
    
    while (1)
    {
      int num_step = get_full_ADC();                          
      int voltage_v = (num_step * 4);                           // multiply count by Vref
      printf ("The Voltage is %f\r\n\n\n", voltage_v/1000.0);   // Print to TeraTerm  
      BCD_Upper_Digit (voltage_v/10000);                        // Set Upper Digit
      BCD_Lower_Digit ((voltage_v/1000)%10);                    // Set Lower Digit
      BCD_DP (1);                                               // Set Decimal Point
      WAIT_1_SEC();                                             // Wait 1 Sec
    }
